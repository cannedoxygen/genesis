...
class IntroScene extends BaseScene {
  ...
  enter() {
    ...
    if (assets.audio && assets.audio.bgm && !assets.audio.bgm.isPlaying()) {
  ...
  showCharacters() {
    ...
    const characterAnimation = setInterval(() => {
      ...
      aikira.setPosition(width * 0.3, y);
      cliza.setPosition(width * 0.7, y);
  ...
  showStartButton() {
    ...
    cliza.setEnergy(80); // Excited
    byte.setAnimationState('alert'); // Alert mode
  }
  ...
  resize() {
    ...
    if (aikira.visible) aikira.setPosition(width * 0.3, height * 0.6);
    if (cliza.visible) cliza.setPosition(width * 0.7, height * 0.6);
    if (byte.visible) byte.setPosition(width * 0.5, height * 0.7);
  }
}
